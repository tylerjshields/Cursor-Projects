...
set window_start = current_date-30; ----'2025-01-01'; --dateadd('d', -30, current_date);
...
select 
    ...
    is_push_reachable_special_offers, is_push_reachable_other,
...
-- Create daily dashboard output with same structure but at a daily grain
-- Daily version - preserves day-by-day metrics
create or replace table proddb.tylershields.nvg_notif_daily_dashboard_v2 as
select 
    sent_at_date,
    team,
...
from proddb.tylershields.nvg_notif_hll_objects_v2
group by sent_at_date, team, notification_channel
